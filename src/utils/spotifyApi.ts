...
export async function getPlaylists(): Promise<SpotifyPlaylist[]> {
  // Fetch up to 50 at a time; paginate if needed
  ...
  while (url) {
    ...
    // `next` is an absolute URL — strip the base to reuse apiFetch
...
export async function searchPublicPlaylists(
  ...
  // Quota. Anything > 10 returns "Invalid limit" 400. Confirmed via live test
  // 2026-05-02.
  ...
  // Spotify's search results sometimes include null entries — filter them out
